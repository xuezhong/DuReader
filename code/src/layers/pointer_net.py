...
def attend_pooling(pooling_vectors, mask, ref_vector, hidden_size, scope=None):
    """
        ...
        with tf.variable_scope('pn_decoder'):
            ...
            if init_with_question:
                ...
                pooled_question_rep = tc.layers.fully_connected(
                    attend_pooling(question_vectors, question_mask, random_attn_vector, self.hidden_size),
            ...
            with tf.variable_scope('fw'):
                fw_cell = PointerNetLSTMCell(self.hidden_size, passage_vectors, passage_mask)
                fw_outputs, _ = custom_dynamic_rnn(fw_cell, fake_inputs, sequence_len, init_state)
            with tf.variable_scope('bw'):
                bw_cell = PointerNetLSTMCell(self.hidden_size, passage_vectors, passage_mask)
