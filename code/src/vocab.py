...
class Vocab(object):
    ...
    def load_pretrained_embeddings(self, embedding_path):
        ...
        """
        if self.embed_dim != self.embeddings.shape[1]:
            self.embeddings = self.embeddings[:, :self.embed_dim]
        # trained_embeddings = {}
        with open(embedding_path, 'r') as fin:
            ...
            for line in fin:
                ...
                self.embeddings[self.get_id(token)][:self.embed_dim] = np.asarray(list(map(float, contents[1:])))
                print('Loading token %s ...' % token)
